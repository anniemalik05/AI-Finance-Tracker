...
def categorize(description):
    categories = {
        ...
        'Dining': ['restaurant', 'coffee', 'fast food', 'dining'],
        'Transport': ['gas', 'fuel', 'taxi', 'transport'],
        'Shopping': ['shopping', 'clothing', 'bookstore', 'electronics'],
...
if os.path.exists(DATA_FILE):
    try:
        ...
        # 2. Add useful time features
        df['Month'] = df['Date'].dt.month_name()
        df['Day_of_Week'] = df['Date'].dt.dayofweek  # 0=Monday
        
        # 3. Clean amounts (already in your code)
...
with tab1:
    ...
    with col3:
        st.markdown("""
            <div class="card">
                <div class="card-title">⚖️ Net Balance</div>
        ...
        if len(expenses_df) >= 2:
            ...
            st.markdown(f"""
                <div style='background: rgba(31, 59, 115, 0.05); padding: 1rem; border-radius: 8px; margin-top: 1rem;'>
                    ...
                    <p style='color: black;'>• Average daily expense: <strong style='color: black;'>Rs.{avg_daily:.2f}</strong></p>
                    <p style='color: black;'>• Predicted expenses for next 7 days: <strong style='color: black;'>Rs.{predicted_expenses.mean():.2f}/day</strong></p>
                    <p style='color: black;'>• Total predicted for next week: <strong style='color: black;'>Rs.{predicted_expenses.sum():.2f}</strong></p>
                </div>
            """, unsafe_allow_html=True)
        else:
